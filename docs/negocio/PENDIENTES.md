...
> | El resumen paginado (25 por página) | **falta empujar el commit** — está en `~/wacrm` |
>
...
>
> **Cerrado a las 06:50 sin que nadie lo pidiera:** `anon` —o sea cualquiera con
> la clave pública del navegador, que va dentro del bundle— podía ejecutar cinco
> funciones del Cerebro sin iniciar sesión, entre ellas
> `cerebro_outbox_reclamar(10)`, que se lleva los mensajes pendientes y dejaría a
> los clientes sin respuesta **en silencio**. Migración 089. No rompe nada: n8n
> entra como `postgres` por Supavisor (comprobado en `pg_stat_activity`).
...
> | 🟢 | **El resumen soltaba 200 filas de un tiro, y a finales de mes iba a cortar en silencio — arreglado; falta empujarlo y desplegarlo.** Lo pidió Humberto el 14-ago viendo la misma enfermedad que la columna «Entregada»: la tabla crece sin fin. Medido: **95 filas en 6 días, ~16 al día**, así que el tope de 200 se alcanzaba **alrededor del 26-ago** — y lo grave no era la tabla larga, era que al llegar ahí la función **corta sin avisar** y las operaciones viejas desaparecen. **Migración 088 (ya en producción):** `p_desde_fila` (OFFSET), columna `total` con las filas del filtro entero, y **desempate en el `ORDER BY`** — sin él, dos filas empatadas cambian de orden entre llamadas y una sale en dos páginas mientras otra no sale en ninguna; hay empates reales (la operación `1054d03b` tiene tres beneficiarios vigentes). Probado ANTES de aplicar: las 4 páginas de 25 sobre las 95 filas reales, **0 sobran, 0 faltan**, `total`=95 en las cuatro. `p_limite` mantiene su DEFAULT 200 **a propósito**, para que la pantalla desplegada ahora mismo siga igual hasta el despliegue. **Migración 088b:** al recrear la función nació con EXECUTE para PUBLIC — `anon` podía leer el historial sin sesión; revocado, ACL letra por letra igual al de antes | **el commit `d91e4ea` está en `~/wacrm`, sin empujar: GitHub devolvió 403 al escribir.** `git -C ~/wacrm push origin master`, y después el Deploy de siempre |
> | 🟢 | **`anon` podía ejecutar cinco funciones del Cerebro sin iniciar sesión — cerrado (089).** Lo destapó el ACL de la 088. Eran `cerebro_outbox_reclamar(integer)`, `cerebro_outbox_encolar`, `cerebro_outbox_resultado`, `cerebro_ejecutar_tool_dsml` y `cerebro_avisar_chats_atascados`. **La peor con diferencia es `outbox_reclamar(10)`**: solo pide un número, y quien la llame se lleva los mensajes pendientes marcados como reclamados — el enviador de verdad no encuentra nada y **los clientes dejan de recibir respuestas en silencio**. Nacieron así después de la 064: su `ALTER DEFAULT PRIVILEGES` no las cubrió (ACL en NULL = EXECUTE para PUBLIC). **Por qué no rompe nada:** n8n entra por conexión directa como rol `postgres` (visto en `pg_stat_activity`, vía Supavisor), no por PostgREST; en todo WaCRM no hay ni una llamada a las cinco (grep en el repo entero); y aun así se les dio GRANT explícito a `authenticated` y `service_role`. El único que pierde acceso es `anon`. Comprobado después: las **46** funciones `cerebro_*` llamables dan `anon = false` | **la lección, que vale para toda migración futura:** el ACL hay que MIRARLO después de crear la función, no darlo por hecho. La consulta está escrita en la 088b |
